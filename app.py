...
def index():
    ...
    stocks = db.execute("SELECT symbol, symbol, price, SUM(shares) as totalShares FROM transactions WHERE user_id = ? GROUP BY symbol HAVING SUM(shares) > 0 ORDER BY SUM(shares) * price DESC", user_id)
